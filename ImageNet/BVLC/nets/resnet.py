...
def resnet_model(inputs, training, resnet_size, bottleneck, num_classes, num_filters,
               kernel_size,
  ...
  second_pool_stride = second_pool_stride
  ...
  # inputs = tf.Print(inputs, [inputs], summarize=10, message=str(training) + '_before_BN')
  for i, num_blocks in enumerate(block_sizes):
    num_filters2 = num_filters * (2 ** i)
    inputs = block_layer(
      inputs=inputs, filters=num_filters2, bottleneck=bottleneck,
  ...
  # inputs = tf.Print(inputs, [inputs], summarize=10, message=str(training) + '_after_BN')
  return layer, inputs
